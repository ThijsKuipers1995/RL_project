...
def q_learning(env, policy, Q, num_episodes, discount_factor=1.0, alpha=0.1, result_target=True, verbatim=True):
    stats = []
    if result_target:
...
def double_q_learning(env, policy, Qt, Qb, num_episodes, discount_factor=1.0, alpha=0.1, random_Q_choice=True, eps=0.5, update_Q_fn=lambda Qt, Qb: Qt + Qb, result_target=True, verbatim=True):
    stats = []
    if result_target:
